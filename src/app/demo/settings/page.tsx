...
import { PUBLIC_DEMO_TIMEZONE } from "@/lib/demo/public-demo-data";
...
export default function DemoSettingsPage() {
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <Card><CardHeader><CardTitle>Why settings matter</CardTitle><CardDescription>Intake measures alignment rather than prescribing a universal plan.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex gap-2 rounded-xl border bg-background/70 p-3 text-sm"><Clock3 className="mt-0.5 size-4 shrink-0 text-primary" /><span>Dashboard and log times are shown in <strong>{PUBLIC_DEMO_TIMEZONE}</strong>.</span></div><ul className="space-y-3 text-sm text-muted-foreground"><li className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-primary" />Effective-dated goals preserve historical context.</li><li className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-primary" />Missing values remain unknown instead of becoming zero.</li><li className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-primary" />Every insight has an explicit evidence threshold.</li></ul><Button asChild className="w-full"><Link href="/login"><LockKeyhole /> Create an account to personalize</Link></Button></CardContent></Card>
    </div>
  </div>;
}
