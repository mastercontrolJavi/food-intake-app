...
export function PeriodView({
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-primary">{eyebrow}</p><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-muted-foreground">{description}</p></div>{navigation && <div className="flex items-center gap-2"><Button variant="outline" size="icon" asChild><Link href={navigation.previousHref} aria-label="Previous period"><ChevronLeft /></Link></Button><Button variant="outline" asChild><Link href={navigation.currentHref}>Current</Link></Button>{navigation.nextHref ? <Button variant="outline" size="icon" asChild><Link href={navigation.nextHref} aria-label="Next period"><ChevronRight /></Link></Button> : <Button variant="outline" size="icon" disabled aria-label="Next period unavailable"><ChevronRight /></Button>}</div>}</div>
