...
export type MealPrefill = {
  ...
  nutritionSource?: string; nutritionExternalId?: string | null; notes?: string | null;
};
...
export function MealForm({ initial }: { initial: MealPrefill }) {
  ...
  return <form action={action} className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-2"><Label htmlFor="quantity">Quantity</Label><Input id="quantity" name="quantity" type="number" min="0.001" max="1000" step="0.001" inputMode="decimal" defaultValue={initial.quantity ?? 1} required /></div>
    ...
    <fieldset><legend className="mb-3 font-medium">Nutrition <span className="font-normal text-muted-foreground">(total for this log; enter only what you know)</span></legend><div className="grid grid-cols-2 gap-4 sm:grid-cols-4">{nutritionFields.map((field) => <div key={field.key} className="space-y-2"><Label htmlFor={field.key}>{field.label}</Label><div className="relative"><Input id={field.key} name={field.key} type="number" min="0" step="0.1" inputMode="decimal" value={nutrition[field.key]} onChange={(event) => set(field.key, event.target.value)} className="pr-12" /><span className="pointer-events-none absolute inset-y-0 right-3 grid place-items-center text-xs text-muted-foreground">{field.suffix}</span></div></div>)}</div></fieldset>
